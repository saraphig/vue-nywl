...
<template>
  ...
  <div class="box">
    <table class="table table-hover table-bordered" cellpadding="0" cellspacing="0">
      <thead>
      <tr class="webfont">
        <th width="100" v-if="check">
          <span class="checkcont"><input type="checkbox" name="" class="checkboxall" id="checkboxall" value=""
                                         @click="checkboxAll()"/><label for="checkboxall"
                                                                        :class="checkStatus=='radio'?'disabled':'' "></label></span>
        </th>
        <th width="150" v-if="tableSorting" class="orderNum">序号</th>
        <th v-for="(val,key) in tabTitle" v-text="val" :width="tabwidth[key]"></th>
        <th v-if="operate" width="250">操作</th>
        <th v-if="ooperate" width="250">设备列表</th>
      </tr>
      </thead>
      <tbody>
      <tr v-for="(data, index) in datalist" :key="data.id">
        <td v-if="check">
          <span class="checkcont"><input type="checkbox" name="" :id="'as'+data.id" class="checkbox" :value="data.id"
                                         @click="checkboxa(data.id,index)"/><label :for="'as'+data.id"></label></span>
        </td>
        <td v-if="tableSorting" v-text="((pageNo-1)*pageLen)+index+1" class="orderNum"></td>
        <td v-for="(val,key) in tabTitle">

						<span v-if="data[key] && data[key].hasOwnProperty('name')">
							<span style="color: #000;" v-if="data[key].type==0" v-text="data[key].name"></span>
	            <span style="color: #EC1561;" v-if="data[key].type==1" v-text="data[key].name"></span>
							<span style="color: #FF9939;" v-if="data[key].type==2" v-text="data[key].name"></span>
							<span style="color: #111A39;" v-if="data[key].type==3" v-text="data[key].name"></span>
						</span>
          <span v-else v-text="data[key]" :style="{width:tabwidth[key] + 'px'}"></span>
    ...
    <div class="v-page" v-if="datalist.length>0">
      ...
      <div class="boot-page" v-if="pageTotal>1">
        ...
        <a href="javascript:void(0)" v-for="val in arrb" v-if="pageNo-val>1"
           @click="onPageClick(pageNo-val)">{{pageNo - val}}</a>
        <a class="active" href="javascript:void(0)">{{pageNo}}</a>
        <a href="javascript:void(0)" v-for="val in arra" v-if="pageNo+val<pageTotal"
           @click="onPageClick(pageNo+val)">{{pageNo + val}}</a>
  ...
  export default {
    ...
    methods: {
      ...
      checkboxa(i, index) {
        ...
        if(this.checkboxs.length!=this.pageLen){
          document.getElementsByClassName("checkboxall")[0].checked=false;
        }else{
    ...
    mounted() {
      ...
      if (this.checkStatus == "radio" && this.check) {
    ...
    watch: {
      ...
      datalist (){
        ...
        if(this.checkStatus=='check' && this.check==true){
          document.getElementsByClassName("checkboxall")[0].checked=false;
        }
      }
    },
