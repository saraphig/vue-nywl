...
export const checkVideo = (obj, fileSize) => {
  ...
  if (fileSize > 500 * 1024) {
    return swal({
      title: '错误提醒',
      text: '您选择的视频大小超过限制(最大为500M)，请重新选择',
      confirmButtonText: "知道了"
    });
  }
  return true;
...
//剪切字串
export const cutStr = (inStr, cutlen) => {
    let charCount = 0;
    let tmpStr = "",resStr = "";
    for (let i = 0; i< inStr.length; i++) {
        ...
        if ((c >= 0x0001 && c <= 0x007e) || (0xff60 <= c && c<=0xff9f)) {
            charCount ++;
            tmpStr = inStr.substr(i,1);
        } else {
            charCount += 2;
            tmpStr = inStr.substr(i,1);
        }
        resStr += tmpStr;
        //console.log(resStr + "   " + charCount);
        if (charCount >= cutlen) break;
    };
    return resStr;
}
...
var AnyTaskList = [];
export const setResizeCallback = (funCallback) => {
    if (typeof funCallback == "function") {
        AnyTaskList.push(funCallback);
    };
    if (AnyTaskList.length <= 1) {
        var docEl = document.documentElement;//document, window,
        var resizeEvt = 'orientationchange' in window ? 'orientationchange' : 'resize';
        var recalc = function () {
            let clientWidth = docEl.clientWidth;
            if (!clientWidth) return;
            //这里是假设在1920px宽度设计稿的情况下，1rem = 16px；
            //可以根据实际需要修改
            docEl.style.fontSize = 100 * (clientWidth / 1920) + 'px';
            document.body.style.fontSize='16px';
            //console.log(docEl.style.fontSize);
            //调用事件回调函数列表
            if (AnyTaskList.length > 0) {
                for (let i = 0; i < AnyTaskList.length; i ++) {
                    let Callfunction = AnyTaskList[i];
                    if (typeof Callfunction == "function") {
                        Callfunction(docEl.style.fontSize,clientWidth);
                    };//end if
                }
            }  
        };
...
//--------------------------------------------------------------------------------------------------------------------
//绑定全局点击事件
export const documentClick = (funCallback) => {
  document.addEventListener("click", e => {
    if (typeof funCallback == "function") {
      funCallback(e);
    } else {
      // console.log(typeof funCallback);
      // console.log(funCallback);
    }
  });
};
//--------------------------------------------------------------------------------------------------------------------
//返回格式化的时间数据; 例: 当前时间 getDateFormat("yyyy-mm-dd hh:nn:ss",new Date());
export const getDateFormat = (fmt,date) => { //author: meizz   ("yyyy-mm-dd hh:nn:ss",new Date()
    var o = {   
