<template>
  <div class="change-password-bg">
    <div class="mp-header-button-group">
      ...
      <a v-if="!isFromPersonalCenter" class="jump-btn-text" @click="jump">跳过</a>
      <p v-else class="empty-space"></p>
    </div>
    <img class="change-password-img" src="../../../assets/img/login/change-password.png"/>
    ...
    <div class="change-password-form">
      ...
      <div class="form-group" :class="{ 'form-group--error': $v.newPassword.$error }">
        <label>
          <i class="iconfont icon-lock"></i>
          <input
            class="form__input"
            ...
            maxlength="12"
            type="password"
            name="chanage-password-new-password"
            placeholder="请输入6-12位密码">
        </label>
      </div>
      <span class="form-group__message" v-if="!$v.newPassword.required">请输入6-12位数密码</span>
      <span class="form-group__message" v-if="!$v.newPassword.minLength">
        请输入{{ $v.newPassword.$params.minLength.min }}到{{ $v.newPassword.$params.maxLength.max }}位数密码
      </span>
      <div class="form-group" :class="{ 'form-group--error': $v.repeatNewPassword.$error }">
        <label>
          <i class="iconfont icon-lock"></i>
          <input
            class="form__input"
            ...
            maxlength="12"
            @keyup.enter="finish"
            name="chanage-password-repeat-new-password"
            placeholder="再次确认密码">
        </label>
      </div>
      <span class="form-group__message" v-if="!$v.repeatNewPassword.required">请输入确认密码</span>
      <span class="form-group__message" v-if="$v.repeatNewPassword.required && !$v.repeatNewPassword.sameAsPassword">两次输入密码不一致，请重新确认</span>
    </div>
    <button class="finish-button" @click="finish" name="chanage-password-finish-button">完成</button>
  </div>
</template>
