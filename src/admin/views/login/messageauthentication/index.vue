<template>
  <div class="forget-password-bg">
    ...
    <img class="forget-password-img" src="../../../assets/img/login/forget-password-step2.png"/>
    <p class="forget-password-step1-text">账户验证</p>
    <div class="forget-password-form">
      <div class="form-group" :class="{ 'form-group--error': $v.authCode.$error }">
        <label>
        ...
        <div :class="!this.sendAuthCode ? 'forget-password-auth-code' : 'forget-password-auth-code-active'">
          <span v-show="sendAuthCode" class="forget-password-auth-code-text" @click="getAuthCode"
                name="meaasge-authentication-get-auth-code">获取验证码</span>
          <span v-show="!sendAuthCode" class="forget-password-auth-code-text">
            重新获取<span>({{auth_time}}S)</span>
          </span>
        </div>
      </div>
      <span class="form-group__message" v-if="!$v.authCode.required">请输入验证码</span>
      <p class="forget-password-step1-tip">四位验证码已发送至{{username}}</p>
    </div>
    <button class="next-step-button" @click="nextStep" name="meaasge-authentication-next-step">下一步</button>
  </div>
</template>
