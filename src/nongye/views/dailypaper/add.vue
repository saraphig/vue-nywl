<template>
	<div class="dailypaper-add-page">
		<div class="bgcolor">
			<div class="dailypaper-content">
				<div class="content-head">
					<div class="page-title">新增日报</div>
					<div class="date-comp">
...
<script>
	...
	export default {
		...
		mounted() {
			// 默认显示当天时间
			this.$store.dispatch("request_daily_currentTime", {}).then((response) => {
				if(this.isString(response)) {
					this.datePicker = response;
				} else {
					this.datePicker = response.time;
				}
			}, (error) => {
				this.datePicker = moment().format('YYYY-MM-DD');
			});
			this.$refs.errorRef.setDefaultSelect(0, "无工作失误/事故"); //默认值：没有工作失误
			this.$refs.attendanceRef.setDefaultSelect(1, "满勤"); //默认值：满勤
			this.$refs.materialRef.setDefaultSelect(0, "无物料使用"); //默认值：无物料使用
			this.$refs.goodsRef.setDefaultSelect(0, "无进出货"); //默认值：无进出货
		},
		methods: {
			//判断对象是否是字符串
			isString(obj) {
			...
			saveHandle() {
				...
				_this.$store.dispatch("request_daily_checkTime", {
					...
					if(timeState == 0) {
						...
						// 新增日报请求
						_this.$store.dispatch("request_daily_add", dailyData).then((response) => {
							_this.$router.push({
								path: '/dailypaper',
								query: {}
							})
