		...
		<div class="bgcolor">
			<div class="warnnotice-content">
				<div class="content-head">
					<div class="page-title">编辑告警通知</div>
						...
						<div class="content">
							<div class="content-left">
								...
								<div class="selectItem">
									...
									<div class="select">
										<base-select ref="alarmRef" :opts="alarmOpts" :placeholderStr="alarmPlaceholderStr" @disp-selectCallBack="alarmSelectCallBack"></base-select>
									</div>
								</div>
								<div class="selectItem">
									<span class="data-label">通知人：</span>
									<div class="select">
										<base-select ref="userRef" :opts="noticeUserOpts" :placeholderStr="noticeUserPlaceholderStr" @disp-selectCallBack="noticeUserSelectCallBack"></base-select>
									</div>
								</div>
								<div class="inputItem">
...
<script>
	...
	export default {
		...
		mounted() {
			...
			_this.$store.dispatch("get_alarms_notice_list").then((response) => {
				let noticesList = response;
				for(let n = 0; n < noticesList.length; n++) {
					if(noticesList[n].id == noticeID) {
						_this.curNoticeData = noticesList[n];
						break;
					}
				}
				console.log(_this.curNoticeData);
				_this.telNoticeChecked = _this.curNoticeData.isSendSms == "1" ? true : false;
				_this.emailNoticeChecked = _this.curNoticeData.isSendEmail == "1" ? true : false;
				_this.wechatNoticeChecked = _this.curNoticeData.isSendWechat == "1" ? true : false;
				_this.ps = _this.curNoticeData.remark;
				...
				_this.$store.dispatch("request_areaDetail", areaParam).then((data) => {
					...
					_this.areaOpts = data;
					_this.$refs.areaSelect.setDefaultSelect(_this.curNoticeData.areaCode, _this.curNoticeData.areaName);
				}, (error) => {
					console.log(error);
				});
				...
				_this.$store.dispatch("request_alarms_level", param).then((data) => {
					// 格式化
					let almLevelID = _this.curNoticeData.almLevel;
					let almLevelName = "";
					for(let i = 0; i < data.length; i++) {
						data[i].value = data[i].name;
						if(data[i].id == almLevelID) {
							almLevelName = data[i].name;
						}
					}
					_this.alarmOpts = data;
					_this.$refs.alarmRef.setDefaultSelect(almLevelID, almLevelName);
				}, (error) => {
					console.log(error);
				});
				...
				_this.$store.dispatch("getUserList", userParam).then((resp) => {
					...
					let userID = _this.curNoticeData.userId;
					let userName = "";
					...
					_this.$refs.userRef.setDefaultSelect(userID, userName);
				}, (error) => {
					console.log(error);
				});
			});
		...
		methods: {
			...
			saveHandle() {
				...
				// 保存实现
				let noticeData = {};
				noticeData.id = _this.curNoticeData.id;
				...
				_this.$store.dispatch("request_alarms_notice_edit", noticeData).then((response) => {
					this.$router.push({
						path: '/page_warnnotice',
						query: {}
					})
