...
function divdisplay(id) {
    switch(lastID) {
        case 0: $("#videodiv").hide(); break;
        case 1: $("#logindiv").hide(); break;
        case 2: $("#previewdiv").hide(); break;
        case 3: $("#ptzdiv").hide(); break;
        case 4: $("#playbackdiv").hide(); break;
        case 5: $("#maintaindiv").hide(); break; 
        case 6: $("#operatediv").hide(); break;
    };
    switch(id) {
        case 0: $("#videodiv").show(); break;
        case 1: $("#logindiv").show(); break;
        case 2: $("#previewdiv").show(); break;
        case 3: $("#ptzdiv").show(); break;
        case 4: $("#playbackdiv").show(); break;
        case 5: $("#maintaindiv").show(); break;
        case 6: $("#operatediv").show(); break;
    };
    lastID = id;
    //console.log(lastID);
    return false;
...
$(function () {
    ...
    let v_width = parseInt(getQueryString("vwidth")) - 20;//699, 471
    let v_height = parseInt(getQueryString("vheight")) -120;
    v_videoIP = getQueryString("videoIP");
    let v_setup =  getQueryString("setup");
	...
	// 初始化插件参数及插入插件
	WebVideoCtrl.I_InitPlugin(v_width, v_height, { //500, 300
...
//////////////////////////////////////////////////////////////////////////////////////////////////>
function PlayAll(){
	var vi= iiTypes * iiTypes;
	var curIndex = 0;
	$("#channels option").each(function() {    
    var op = $(this).val();
	if(curIndex <vi )
	{
		GetSelectWndInfo('<?xml version="1.0"?><RealPlayInfo><SelectWnd>'+curIndex+'</SelectWnd><ChannelNumber>-1</ChannelNumber><RealPlaying>0</RealPlaying><Recording>0</Recording></RealPlayInfo>');
		var oWndInfo = WebVideoCtrl.I_GetWindowStatus(curIndex);
		var szIP = $("#ip").val();
		var iStreamType = parseInt($("#streamtype").val(), 10); //码流类型
		var iChannelID = op; //当前窗口
...
function clickStartPlayback() {
	var oWndInfo = WebVideoCtrl.I_GetWindowStatus(g_iWndIndex),
		...
		iRet = -1;
	console.log(szIP,szStartTime,szEndTime,iChannelID);
	if ("" == szIP) {
		return;
	}
